...
jint Java_com_snailstudio_xsdk_debug_serv_terminal_TermWebSocket_socket(JNIEnv *env, jobject thiz) {
    printf("Java_com_snailstudio_xsdk_debug_serv_terminal_TermWebSocket_socket\n");
    ser_fd = socket(AF_INET, SOCK_STREAM, 0);
    return passive_server(ser_fd, 20);
}

jint Java_com_snailstudio_xsdk_debug_serv_terminal_TermWebSocket_accept(JNIEnv *env, jobject thiz) {
    printf("Java_com_snailstudio_xsdk_debug_serv_terminal_TermWebSocket_accept\n");
    struct sockaddr_in client_addr;
    socklen_t addr_length = sizeof(client_addr);
...
void Java_com_snailstudio_xsdk_debug_serv_terminal_TermWebSocket_run(JNIEnv *env, jobject thiz) {

    printf("Java_com_snailstudio_xsdk_debug_serv_terminal_TermWebSocket_run\n");
    while (1) {
        frame_head head;
        int rul = recv_frame_head(conn, &head);
        if (rul < 0) {
        ...
        do {
            ...
            jclass TestProvider = (*env)->FindClass(env,
                                                    "com/snailstudio/xsdk/debug/serv/terminal/TermWebSocket");
...
Java_com_snailstudio_xsdk_debug_serv_terminal_TermWebSocket_send(JNIEnv *env, jobject obj,
                                                                 jbyteArray jdata,
                                                                 jint jlength) {
